...
(* - needs to hav some form of "grid" such that we can have multiple lines *)
(* - functions revealed to the "client" will only involve methods that already optimize or deoptimize a line  *)
(* - no need to care about colors for now, not usre if I can easily implement that as of now *)
